...
### Post-Phase 5 Optimization Tasks (Deferred)
1. Response caching ($10-30/month savings)
2. Container right-sizing ($10-20/month savings)
3. Fine-tuned models ($15-30/month savings)
4. Self-hosted vector search ($5-10/month savings)
5. UCP Integration (+$15-25/month, 80-120 hours)
...
## CURSOR Report (CI/CD Health Check)

### What is the CURSOR Report?

The **CURSOR Report** (`CURSOR-EVALUATION-REPORT.md`) is a comprehensive CI/CD and reproducibility audit generated by Cursor AI. It performs static analysis of the repository to identify configuration drift, broken CI pipelines, and reproducibility issues.

### When to Generate

The user prefers **manual triggering** of this report. When asked "should we run a CURSOR report?" or "is it time for a CURSOR report?", consider these factors:

| Generate Report When | Skip Report When |
|---------------------|------------------|
| Before merging to main | Trivial doc-only changes |
| After major refactors | WIP commits on feature branches |
| CI failures of unknown cause | Changes only to gitignored files |
| Before releases/tags | Rapid iteration on known issues |
| Significant config file changes | Report was run recently (<1 week) |

### What the Report Covers

1. **Python Version Consistency** - Checks ci.yml, azure-pipelines.yml, pytest.ini, docs
2. **Docker Service Names** - Validates compose service names match CI workflow references
3. **Gitignore vs CI Validation** - Ensures CI doesn't check for gitignored files
4. **pytest.ini Coupling** - Checks for mandatory plugin dependencies
5. **Security Scan Summaries** - Reviews Bandit and OWASP ZAP findings
6. **Documentation Drift** - Identifies contradictions between docs and configs

### Report Location

- **File:** `CURSOR-EVALUATION-REPORT.md` (root directory)
- **Generated by:** Cursor AI (external tool)
- **Reviewed by:** Claude (this assistant)

### Historical Issues Found (2026-01-28)

| Issue | Severity | Status |
|-------|----------|--------|
| Python 3.14 in CI (unavailable on runners) | P0 | ✅ FIXED |
| Docker service name mismatch (slim-service, mock-analytics) | P0 | ✅ FIXED |
| Gitignored files validated in dev-ci.yml | P1 | ✅ FIXED |
| pytest-cov mandatory in pytest.ini | P1 | ✅ FIXED |
| Windows/OneDrive troubleshooting missing | P2 | ✅ FIXED |

### How to Request

Simply ask: *"Should we run a CURSOR report?"* or *"Is it time for a CURSOR report?"*

The assistant will evaluate based on recent changes and recommend whether a report would be valuable.
