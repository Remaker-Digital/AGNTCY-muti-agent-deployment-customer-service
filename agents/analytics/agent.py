...
class AnalyticsAgent(BaseAgent):
    ...
    def get_demo_messages(self) -> List[dict]:
        ...
        for event_data in [
            ...
            },
            {
                ...
                "agent_source": "knowledge-retrieval",
            ...
            },
            {
                ...
                "metrics": {
                    ...
                    "reason": "Standard query - automated",
                },
            },
        ]:
            events.append(
    ...
    def _update_kpis(self, event: AnalyticsEvent):
        ...
        # Track intent classification
            ...
            self.kpis["intents_classified"][intent] = (
                self.kpis["intents_classified"].get(intent, 0) + 1
            )
    ...
    def _log_kpi_summary(self):
        ...
        avg_response_time = (
            (self.kpis["total_response_time_ms"] / total_conv) if total_conv > 0 else 0
        )

        # Calculate average confidence
        avg_confidence = (
            (sum(self.kpis["avg_confidence"]) / len(self.kpis["avg_confidence"]))
            if self.kpis["avg_confidence"]
            else 0
        )
        ...
        self.logger.info(
            f"Escalated: {escalated} ({(escalated/total_conv*100) if total_conv > 0 else 0:.1f}%)"
        )
    ...
    def get_kpi_report(self) -> Dict:
        ...
        return {
            ...
            "automation_rate_percent": (
                (automated / total_conv * 100) if total_conv > 0 else 0
            ),
            "escalation_rate_percent": (
                (escalated / total_conv * 100) if total_conv > 0 else 0
            ),
            "auto_approvals": self.kpis["auto_approvals"],
            "avg_response_time_ms": (
                (self.kpis["total_response_time_ms"] / total_conv)
                if total_conv > 0
                else 0
            ),
            "avg_confidence": (
