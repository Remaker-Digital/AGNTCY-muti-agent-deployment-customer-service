...
try:
    ...
    from shared.models import (
        CustomerMessage, IntentClassificationResult, KnowledgeQuery,
        ResponseRequest, EscalationDecision, AnalyticsEvent,
        create_a2a_message, extract_message_content, generate_message_id,
        Intent, Language
    )
...
class AGNTCYIntegration:
    ...
    async def initialize_agent_connection(self) -> bool:
        ...
        try:
            ...
            # Create A2A client
            self.client = self.factory.create_a2a_client("console-client", self.transport)
            if not self.client:
    ...
    async def send_customer_message(self, message: str, session_id: str, 
        ...
        try:
            ...
            if AGNTCY_IMPORTS_AVAILABLE:
                customer_msg = CustomerMessage(
                    ...
                    customer_id=customer_context.get('customer_id', 'console-user') if customer_context else 'console-user',
                    content=message,
                    channel='console',
            ...
            message_id = customer_msg.message_id if hasattr(customer_msg, 'message_id') else customer_msg.get('message_id', generate_message_id())
    ...
    async def _send_via_agents(self, customer_msg: CustomerMessage, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.client:
                try:
                    ...
                    intent_response = await self.client.send_message(intent_topic, a2a_message)
                    ...
                    knowledge_context = {}
                    if intent_result['intent'] in ['order_status', 'product_info', 'return_request']:
                        knowledge_response = await self._call_knowledge_agent(customer_msg, intent_result, session_id)
                        knowledge_context = extract_message_content(knowledge_response)
                    
                    # Generate response
                    response_data = await self._call_response_agent(customer_msg, intent_result, knowledge_context, session_id)
                    
                    # Check escalation
                    escalation_data = await self._call_escalation_agent(customer_msg, intent_result, response_data, session_id)
                    
                    # Record analytics
                    await self._call_analytics_agent(customer_msg, intent_result, response_data, escalation_data, session_id)
                    ...
                    return {
                        ...
                        'trace_id': trace_id,
                        'agents_involved': ['intent-classifier', 'knowledge-retrieval', 'response-generator', 'escalation', 'analytics'],
                        'escalation_needed': escalation_data.get('should_escalate', False)
                    }
                    
                except Exception as agent_error:
                    self.logger.warning(f"Real agent communication failed: {agent_error}, falling back to simulation")
                    # Fall back to simulation if real agents fail
                    response_data = await self._simulate_agent_pipeline(customer_msg, trace_id, session_id)
            else:
    ...
    async def _call_knowledge_agent(self, customer_msg: CustomerMessage, intent_result: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    async def _call_response_agent(self, customer_msg: CustomerMessage, intent_result: Dict[str, Any], knowledge_context: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        ...
        response = await self.client.send_message("response-generator-en", response_message)
        return extract_message_content(response)
    
    async def _call_escalation_agent(self, customer_msg: CustomerMessage, intent_result: Dict[str, Any], response_data: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        ...
        response = await self.client.send_message("escalation-handler", escalation_message)
        return extract_message_content(response)
    
    async def _call_analytics_agent(self, customer_msg: CustomerMessage, intent_result: Dict[str, Any], response_data: Dict[str, Any], escalation_data: Dict[str, Any], session_id: str):
    ...
    async def _simulate_agent_pipeline(self, customer_msg: CustomerMessage, trace_id: str, session_id: str) -> Dict[str, Any]:
        ...
        if intent_result['intent'] == 'loyalty_program':
            ...
            # Handle both CustomerMessage object and dict
            customer_id = customer_msg.customer_id if hasattr(customer_msg, 'customer_id') else customer_msg.get('customer_id')
            knowledge_context = {
                'customer_id': customer_id
            }
            agents_involved.append('knowledge-retrieval')
        elif intent_result['intent'] in ['order_status', 'product_info', 'return_request']:
            await asyncio.sleep(0.8)  # Simulate 800ms processing
            knowledge_context = self._retrieve_knowledge(customer_msg.content, intent_result['intent'])
            agents_involved.append('knowledge-retrieval')
        else:
            knowledge_context = {}
        ...
        response = self._generate_response(customer_msg.content, intent_result, knowledge_context)
        agents_involved.append('response-generator')
        
        # Step 4: Escalation Check
        await asyncio.sleep(0.3)  # Simulate 300ms processing
        escalation_needed = self._check_escalation(customer_msg.content, intent_result, response)
    ...
    def _classify_intent(self, message: str) -> Dict[str, Any]:
        ...
        if any(word in message_lower for word in profanity_words):
            return {'intent': 'escalation_needed', 'confidence': 0.95, 'escalation_reason': 'customer_frustration'}

        # Extreme frustration indicators
        frustration_phrases = [
            'this is ridiculous', 'absolutely ridiculous', 'fucking ridiculous',
            'terrible service', 'worst service', 'i hate', 'this sucks'
        ]
        if any(phrase in message_lower for phrase in frustration_phrases):
            return {'intent': 'escalation_needed', 'confidence': 0.92, 'escalation_reason': 'customer_frustration'}

        # Priority 1: Loyalty Program (Issue #34)
        # E2E Tests S005, S006, S007: Loyalty program queries
        if any(word in message_lower for word in [
            "loyalty", "rewards", "points", "loyalty program", "reward program",
            "how many points", "check points", "points balance", "redeem", "tier"
        ]):
        ...
        if any(word in message_lower for word in [
            "subscription", "my subscription", "change subscription", "modify subscription",
            "switch subscription", "cancel subscription", "pause subscription", "auto-delivery", "auto delivery"
        ]):
        ...
        if any(word in message_lower for word in [
            ...
            if any(word in message_lower for word in ["monthly orders", "bulk", "volume", "business", "office", "wholesale"]):
                return {'intent': 'escalation_needed', 'confidence': 0.93, 'escalation_reason': 'b2b_pricing_inquiry'}
        ...
        if any(word in message_lower for word in [
            ...
            "office blend", "team", "our company", "we need", "net 30", "corporate"
        ]):
            return {'intent': 'escalation_needed', 'confidence': 0.92, 'escalation_reason': 'b2b_sales_opportunity'}

        # Priority 5: Return/refund (before order status)
        # E2E Tests S003, S004: Return requests
        if any(word in message_lower for word in ['return', 'refund', 'exchange', 'send back', 'money back']):
        ...
        if any(word in message_lower for word in [
            "where is my order", "where's my order", "track", "tracking", "shipped",
            "delivery", "status of order", "order status", "been delivered", "has my order"
        ]):
            return {'intent': 'order_status', 'confidence': 0.95}

        # Coffee-specific intents
        elif any(word in message_lower for word in ['processing', 'process', 'washed', 'natural', 'honey', 'fermentation']):
        ...
        elif any(word in message_lower for word in ['bitter', 'sour', 'grind', 'extraction', 'v60', 'pour', 'brew', 'french press', 'espresso', 'chemex']):
        ...
        elif any(word in message_lower for word in ['gift', 'recommend', 'suggestion', 'beginner', 'starbucks', 'good for', 'good coffee for']):
            return {'intent': 'product_recommendation', 'confidence': 0.85}
        elif any(word in message_lower for word in ['shipping', 'when', 'how long', 'how fast']):
    ...
    def _generate_response(self, message: str, intent_result: Dict[str, Any], 
        ...
        if intent == 'processing_method':
            ...
            if 'bitter' in message_lower and 'v60' in message_lower:
                ...
                response += "• **Faster pour**: Complete your brew in 3-4 minutes total\n"
                response += "• **Less agitation**: Gentle, steady pours in the center\n\n"
                ...
                response += "- **Grind**: Medium-fine for pour-over, coarse for French press\n"
        ...
        elif intent == 'product_comparison' and 'yirgacheffe' in message_lower and 'sidamo' in message_lower:
            ...
            response = f"Hi! Great news about your order {knowledge_context['order_id']}!\n\n"
            response += f"It {knowledge_context['status']} with {knowledge_context['carrier']} "
            response += f"and should arrive by {knowledge_context['expected_delivery']}.\n"
            ...
            response += f"For best results, try {knowledge_context['brewing_method']}.\n\n"
            ...
            persona_loyalty_data = {
                'persona_001': {'name': 'Sarah', 'points': 475, 'tier': 'Bronze', 'to_silver': 25},
                'persona_002': {'name': 'Mike', 'points': 1250, 'tier': 'Gold', 'auto_delivery': True},
                'persona_003': {'name': 'Jennifer', 'points': 150, 'tier': 'Bronze', 'to_silver': 350},
                'persona_004': {'name': 'David', 'points': 680, 'tier': 'Silver', 'to_gold': 320}
            }

            # Check if we have customer context for personalization
            customer_id = knowledge_context.get('customer_id')
            loyalty_data = persona_loyalty_data.get(customer_id) if customer_id else None
            ...
            elif escalation_reason == 'b2b_sales_opportunity' or escalation_reason == 'b2b_pricing_inquiry':
                response = "Thanks for your interest in our business solutions!\n\n"
                response += "I'm connecting you with our B2B sales team who can provide:\n"
    ...
    def _check_escalation(self, message: str, intent_result: Dict[str, Any], 
        ...
        if any(word in message_lower for word in ['missing', 'stolen', 'never received']):
            return {'should_escalate': True, 'reason': 'Missing delivery investigation required'}
        
        if any(word in message_lower for word in ['defect', 'broken', 'damaged']):
            return {'should_escalate': True, 'reason': 'Product defect within 14-day window'}
        ...
        if any(word in message_lower for word in ['terrible', 'awful', 'frustrated', 'angry']):
    ...
    def _record_analytics(self, customer_msg: CustomerMessage, intent_result: Dict[str, Any],
                         response: Dict[str, Any], escalation_result: Dict[str, Any]):
        """Record analytics event."""
        # Handle both CustomerMessage object and dict
        context_id = customer_msg.context_id if hasattr(customer_msg, 'context_id') else customer_msg.get('context_id', 'unknown')
        message_id = customer_msg.message_id if hasattr(customer_msg, 'message_id') else customer_msg.get('message_id', 'unknown')
    ...
    async def _generate_mock_response(self, customer_msg, session_id: str) -> Dict[str, Any]:
        """Generate a mock response when agents are not available."""
        # Handle both CustomerMessage object and dict
        message_id = customer_msg.message_id if hasattr(customer_msg, 'message_id') else customer_msg['message_id']
        trace_id = str(uuid.uuid4())

        # Use the complete agent pipeline simulation for proper handling
        pipeline_result = await self._simulate_agent_pipeline(customer_msg, trace_id, session_id)
    ...
    def get_conversation_traces(self, session_id: str, limit: int = 50) -> List[ConversationTrace]:
    ...
    def _get_traces_from_clickhouse(self, session_id: str, limit: int) -> List[ConversationTrace]:
        ...
        try:
            ...
            traces = []
            for row in results:
                traces.append(ConversationTrace(
                    trace_id=row[0],
                    span_id=row[1],
    ...
    def _get_mock_traces(self, session_id: str, limit: int) -> List[ConversationTrace]:
        ...
        trace_steps = [
            ('intent-classifier', 'classify_intent', 150, 'Where is my order?', 'order_status'),
            ('knowledge-retrieval', 'search_orders', 800, 'order_status + customer_context', 'Order #12345 found'),
            ('response-generator', 'generate_response', 1200, 'order_status + order_data', 'Your order shipped yesterday...'),
            ('escalation', 'check_escalation', 300, 'response + context', 'no_escalation_needed'),
            ('analytics', 'record_event', 100, 'conversation_data', 'event_recorded')
        ]
        
        trace_id = str(uuid.uuid4())
        
        for i, (agent, action, duration, input_text, output_text) in enumerate(trace_steps):
            start_time = base_time + timedelta(milliseconds=sum(step[2] for step in trace_steps[:i]))
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        for service in mock_services:
            try:
                response = requests.get(f"http://localhost:800{mock_services.index(service) + 1}/health", timeout=5)
        ...
        for agent_name, metrics in agent_metrics.items():
            ...
            health_status['agents'][agent_name] = {
                'status': 'healthy' if success_rate > 0.95 else 'degraded' if success_rate > 0.90 else 'unhealthy',
        ...
        for category in ['services', 'infrastructure', 'agents']:
            all_statuses.extend([item['status'] for item in health_status[category].values()])
